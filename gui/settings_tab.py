...
class SettingsTab(ctk.CTkFrame):
    ...
    def build_ui(self):
        ...
        self.tabview.grid(row=0, column=0, sticky="nsew", padx=SPACING["lg"], pady=SPACING["lg"])
        self.tabview.add("Config")
        self.tabview.add("Test")

        config_tab = self.tabview.tab("Config")
        test_tab = self.tabview.tab("Test")
        config_tab.grid_columnconfigure(0, weight=1)
        test_tab.grid_columnconfigure(0, weight=1)

        self.scroll_frame = ctk.CTkScrollableFrame(config_tab, fg_color=COLORS["panel"])
    ...
    def _build_test_tab(self, parent):
        ...
        card.grid(row=0, column=0, padx=SPACING["lg"], pady=SPACING["lg"], sticky="nsew")
        card.grid_columnconfigure(0, weight=1)

        ctk.CTkLabel(
            card,
            text="Test logica valvole",
            font=font(size=FONT_SIZES["md"], weight="bold"),
            text_color=COLORS["text"],
        ).grid(row=0, column=0, sticky="w", padx=SPACING["md"], pady=(SPACING["sm"], SPACING["xs"]))
        ...
        run_btn.grid(row=1, column=0, padx=SPACING["md"], pady=(0, SPACING["sm"]), sticky="w")

        self.test_log = ctk.CTkTextbox(
            card,
            width=640,
            height=260,
            fg_color=COLORS["panel_alt"],
        ...
        self.test_log.grid(row=2, column=0, padx=SPACING["md"], pady=(0, SPACING["md"]), sticky="nsew")
    ...
    def _write_test_log(self, message):
        if not hasattr(self, "test_log") or self.test_log is None:
            return

        def update_text():
            self.test_log.configure(state="normal")
            self.test_log.delete("1.0", "end")
            self.test_log.insert("end", message)
            self.test_log.configure(state="disabled")
    ...
    def run_valve_logic_test(self):
        def _run():
            cmd = [sys.executable, "debug/test_valve_logic.py"]
            try:
                result = subprocess.run(cmd, capture_output=True, text=True, check=False)
                output = (result.stdout or "") + (result.stderr or "")
                status = f"\nExit code: {result.returncode}"
                self._write_test_log(output + status)
            except Exception as exc:
                self._write_test_log(f"Errore test: {exc}")
